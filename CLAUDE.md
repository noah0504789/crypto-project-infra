...
| `service/docker-compose.yml` | 백엔드 서비스 compose 정의. **CD 배포는 세 전략(blue/green·validated-recreate·safe-recreate) 모두 스크립트가 `docker run`으로 직접 띄운다 — compose를 쓰는 배포 스크립트는 없다.** compose 정의는 로컬/수동 용도이며 스크립트 값과 중복되어 드리프트 주의. 구조 상세는 `docs/SERVICE.md`. |
